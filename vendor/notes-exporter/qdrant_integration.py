...
def _is_qdrant_cloud_url(url: str) -> bool:
    try:
        hostname = (urlparse(url).hostname or "").lower().rstrip(".")
    except Exception:
        return False
    return hostname == "qdrant.io" or hostname.endswith(".qdrant.io")
...
def check_prerequisites(config: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if _is_qdrant_cloud_url(config["qdrant_url"]) or config.get("qdrant_api_key"):
